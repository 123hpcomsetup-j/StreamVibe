...
export default function GuestStream() {
  ...
  const [guestName, setGuestName] = useState(""); // Will be auto-generated by server
  const [message, setMessage] = useState("");
  const [timeLeft, setTimeLeft] = useState(300); // 5 minutes
  const [tokensLeft, setTokensLeft] = useState(100);
  const [guestSessionId, setGuestSessionId] = useState<string | null>(null);
  const [isNameSet, setIsNameSet] = useState(false); // Will be set automatically
  ...
  const createSessionMutation = useMutation({
    ...
    onSuccess: (session: any) => {
      ...
      setGuestName(session.guestName || 'Guest'); // Set auto-generated name
      setIsNameSet(true); // Name is automatically set
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault();
    if (!message.trim() || !guestSessionId) {
      return;
    }
